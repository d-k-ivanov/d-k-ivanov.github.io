...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (iMouseL.z < 0.0f)
    {
        m = vec2(cos(iTime * 0.2f), sin(iTime * 0.2f));
    }
    ...
    for(i = 0; i < maxSteps; i++)
    {
        ...
        position.xy *= rot2D(totalDistance * 0.2f * m.x);           // rotate ray around Z (twisting ray effect)
        position.y += sin(totalDistance * (m.y + 1.0f)) * 0.35f;    // wiggle ray (wavy distortion effect)
    ...
    finalPixelColor = palette(totalDistance * 0.04f + float(i) * 0.005f);
