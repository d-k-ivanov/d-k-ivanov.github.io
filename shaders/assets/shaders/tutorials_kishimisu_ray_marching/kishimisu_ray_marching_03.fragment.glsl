...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for(int i = 0; i < maxSteps; i++)
    {
        ...
        // Debug: color based on number of steps
        finalPixelColor = vec3(float(i)) / float(maxSteps);

        // early stop if we are close enough to the surface
        if (distanceToScene < thresholdNear)
        {
            // Hit the surface
            // finalPixelColor = vec3(1.0f, 0.5f, 0.2f);
            break;
        }

        // early stop if we are too far away
        if (totalDistance > thresholdFar)
        {
            // Missed the surface
            // finalPixelColor = vec3(0.0f);
            break;
        }
    }
