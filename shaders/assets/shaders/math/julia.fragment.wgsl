...
const BASE_ZOOM_DIVISOR : f32 = 2.0;            // Base zoom: screen height divided by this value.
const BASE_CENTER : vec2f = vec2f(0.0, 0.0);    // View center in the complex plane.
const JULIA_C : vec2f = vec2f(-0.8, 0.156);     // Julia constant c defining the fractal.
...
fn juliaColor(z0 : vec2f) -> vec3f
{
    ...
    loop
    {
        ...
        z = vec2f(z.x * z.x - z.y * z.y, 2.0 * z.x * z.y) + JULIA_C;
        iter += 1u;
    }
